...
class Detector(AbstractDetector):
    ...
    #                 for j in range(param.shape[3]):
    #                     slice_ = param[:, :, i, j]
    #                     features.extend(get_feature(slice_))
    #             return features
    #         elif param.dim() == 2:
    #             w = param.view(-1)
    #             w_hist = get_dist(w)
        ...
        for key, tensor in model_state.items():
            ...
            if tensor.dim() == 4 and key.endswith('.weight'):
                ...
                try:
                    ...
                    s = torch.zeros(reshaped_weight.size(0), device=tensor.device)
                num_svd_features = min(10, s.size(0))
                svd_features = s[:num_svd_features]
                if num_svd_features < 10:
                    padding = torch.zeros(10 - num_svd_features, device=s.device)
                ...
                # Pad with zeros for layers without SVD features
                svd_padding = torch.zeros(13, device=flat_tensor.device)
                stats = torch.cat([stats, svd_padding])

            layer_stats.append(stats)

        if layer_stats:
            layer_stats = torch.stack(layer_stats)  # Shape: (num_layers, 217)
            # Compute summary statistics over layers
            features_mean = layer_stats.mean(dim=0)
            features_max = layer_stats.max(dim=0)[0]
            features_min = layer_stats.min(dim=0)[0]
            ...
            # Handle case where there are no layers
            features.append(torch.empty(0, device=next(iter(model_state.values())).device))

        if features:
            model_feats = torch.cat(features).unsqueeze(0)
        else:
            model_feats = torch.empty((1, 0), device=next(iter(model_state.values())).device)
        return model_feats
    ...
    #         for model_dict in model_repr_dict['dict']
    #     ])
    #     y = np.array(model_ground_truth_dict['dict'])
    ...
    def manual_configure(self, models_dirpath: str):
        ...
        for epoch in range(num_epochs):
            ...
            # Optional: Print training loss only
